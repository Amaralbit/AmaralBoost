...
- Perfil Gamer expandido: além de energia e Game Mode, aplica um pacote curado de 9 ajustes individuais + 1 limpeza (definido em `GAMER_BUNDLE`, em `src/tweaks.js`), inspirado no que apps como o Dilera Boost vendem como modo competitivo — sem nada trancado atrás de pagamento, e com cada item listado na tela de revisão antes de confirmar. Um item bloqueado (ex.: exige administrador) não impede os demais.
